...
\title{The memory principle: From simple systems to universal computation}
...
\begin{abstract}
Modern AI systems and biological cells share a fundamental computational constraint: their parallel architectures inherently limit their computational power to TC0 complexity. Yet both types of systems achieve universal computation through a common solution: implementing memory systems. We prove that any TC0-bounded system becomes capable of universal computation when equipped with basic memory capabilities---the ability to maintain state and access computational history. This unifying result connects seemingly disparate phenomena across artificial and biological systems, from the effectiveness of chain-of-thought prompting in language models to state maintenance in cellular processes. Our analysis reveals that the path to more capable AI systems may lie not in increasing model size or complexity, but in developing more robust mechanisms for state maintenance and memory access.
\end{abstract}
