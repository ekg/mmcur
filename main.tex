...
\begin{abstract}
We demonstrate that universal computation emerges naturally from two fundamental capabilities present in both artificial and biological systems: recursive state maintenance and reliable history access. While parallel-trained neural networks and individual biological cells are limited to $\text{TC}_0$ complexity in single forward passes, we prove that systems with these two capabilities can implement a Universal Turing Machine (UTM) with only logarithmic overhead. This result bridges artificial and biological computation, showing how similar computational constraints lead to analogous solutions across domains. Our analysis reveals that the logarithmic overhead in the complexity bounds isn't an implementation artifact but stems from fundamental state encoding requirements. These insights have practical implications for AI system design and help explain why certain interaction patterns prove particularly effective with large language models.
\end{abstract}
