...
\begin{abstract}
We prove that memory mechanisms are sufficient to enable universal computation in parallel systems.
...
This explains how language models perform step-by-step reasoning: not through sophisticated processing units, but by learning to use language itself as a memory system.
This theoretical framework unifies our understanding of computation across minds, machines, and living systems while suggesting that AI capabilities emerge from learning human patterns of state maintenance through language.
\end{abstract}
...
The success of large language models, particularly autoregressive models \cite{schuurmans2024autoregressive}, demonstrates how systems achieve universal computation by learning the fundamental transition functions humans use with language.
These models are trained on vast collections of text that capture how humans use language as a memory system---written explanations, logical arguments, mathematical derivations, and other records of human computational processes \cite{brown2020language}.
This training enables them to internalize not just patterns, but the basic transition functions that humans use to maintain and manipulate computational state through language \cite{wei2022chain}.
