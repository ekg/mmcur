...
\vspace{0.5em}
\noindent\textbf{State Encoding:} Each configuration of $M$ is encoded as a tuple $s = (q, p, a, t)$ where:
\begin{itemize}
    \item $q \in Q$ is the current machine state
    \item $p \in \mathbb{N}$ is the head position
    \item $a \in \Gamma$ is the symbol under the head
    \item $t \in \mathbb{N}$ is the step counter
\end{itemize}
This encoding ensures efficient state validation and access \cite{boyle2024memory,hu2014computational}.

\vspace{0.5em}
\noindent\textbf{Tape Simulation:} For each position $p$ visited by $M$, we maintain a history entry $h_p = (p, a_p, t_p)$ where:
\begin{itemize}
    \item $a_p \in \Gamma$ is the symbol written at position $p$
    \item $t_p$ is the time step when this symbol was written
\end{itemize}
...
\begin{itemize}
    \item $O(\log t)$ bits for step counter
    \item $O(\log n)$ bits for position encoding
\end{itemize}

\noindent\textit{Time complexity:}
\begin{itemize}
    \item $O(\log t)$ for history access operations
\end{itemize}
