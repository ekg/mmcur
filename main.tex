...
\begin{abstract}
Modern AI systems and biological cells share a fundamental characteristic: they are built on parallel computational foundations that limit their complexity in single processing steps. However, both types of systems transcend these limitations by implementing memory systems that maintain state across multiple steps. We prove that any system with basic memory capabilities---the ability to maintain state and access computational history---becomes capable of universal computation, regardless of its underlying parallel architecture. This result explains how seemingly simple systems achieve complex sequential reasoning, from neural networks to cellular processes. Our analysis provides practical insights for AI system design, revealing why certain interaction patterns (like chain-of-thought prompting) are particularly effective with language models, and suggests new approaches for developing more capable AI architectures based on robust state maintenance rather than increased model size.
\end{abstract}
