...
\begin{abstract}
Memory lies at the heart of computational expressivity.
The ability to maintain and access computational state transforms simple systems into universal computers.
This principle manifests across scales - from molecular processes to neural networks - where basic parallel operations combine with state maintenance to enable complex computation.
We show how humanity's own computational evolution traces this pattern: through oral traditions, writing, and now digital systems, we have progressively expanded our capacity to maintain and manipulate computational state.
Modern language models mirror this evolution, learning to replicate human computational patterns by internalizing the basic transition functions embedded in our recorded traces of thought.
This theoretical framework reveals a profound unity: from cells to silicon, universal computation emerges not from complexity of operation, but from the simple power to remember.
\end{abstract}
