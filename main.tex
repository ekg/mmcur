...
\begin{abstract}
We examine how computational universality emerges from the combination of constant-depth parallel computation with reliable memory access across well-defined temporal episodes. This framework unifies our understanding of how parallel systems---from neural networks to biological cells---transcend their inherent computational limitations. While these systems are restricted to constant-depth parallel computations within each episode, they can achieve universal computation by recursively updating and remembering state across episodes. We connect these principles to diverse phenomena, from the effectiveness of chain-of-thought reasoning in language models to information processing in organisms. This suggests that computational expressiveness emerges not from complex processing units, but from mechanisms for maintaining and accessing recursive state updates across well-defined temporal boundaries.
\end{abstract}
