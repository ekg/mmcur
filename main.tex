...
These systems share a common principle of memory-enabled computation (see Table~\ref{tab:MemoryComparison}).
...
\begin{tabular}{lcc}
\toprule
\textbf{System} & \textbf{Recursive State Maintenance} & \textbf{Reliable History Access} \\
\midrule
Cells (genetics, epigenetics) & Yes (DNA replication, gene regulation) & Not fully (indefinite addressability lacking) \\
Neural Networks (RNNs) & Yes, if hidden states persist & Partially (depends on stable recurrence) \\
Transformers (context) & Yes, via context re-injection & Yes, up to window size \\
\bottomrule
\end{tabular}
\caption{\small Comparison of selected systems against the memory criteria defined in Section~2. 
Cells exhibit partially reliable memory but lack unlimited addressable history. 
RNNs can maintain recursive states if properly designed, but are limited by capacity. 
Transformers simulate recursion by reintroducing previous tokens in context.}
\label{tab:MemoryComparison}
\end{table}
