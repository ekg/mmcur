...
The same memory mechanism that preserves logical continuity from step to step also ensures continuous policy adherence, preventing the system from losing alignment constraints in the midst of multi-step reasoning.
...
In language models, as in many neural-based systems, there is a fundamental tension between pattern matching---where the model recognizes or replicates textual patterns in a single pass---and multi-step reasoning, which hinges on robust state maintenance across multiple inference steps.
This tension directly reflects our principle that memory is indispensable for universal computation.
When a model can only match patterns, it remains limited to parallel inference without harnessing the power of sequential logic.
In contrast, chain-of-thought or memory-based approaches allow the system to accumulate partial results, retrieve them as needed, and thereby execute more advanced computations.
