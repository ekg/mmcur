...
Our analysis unifies understanding of computation across biological systems, artificial intelligence, and human cognition.
\end{abstract}
...
These parallel processing constraints fundamentally limit the network's sequential computation abilities \cite{wei2022chain}.
While neural networks might seem to achieve sequential computation through learned representations, theoretical work proves that such implicit encoding cannot circumvent the parallel training limitations \cite{merrill2023parallelism}.
The $\text{TC}_0$ bound emerges specifically from examining the backwards pass during training \cite{jung2020new}.
Any architecture implementing arbitrary sequential computation in a single forward pass would require propagating gradients through this sequential computation.
This would create an inherently serial process that conflicts with parallel training requirements \cite{zhu2024overcoming}.

Recent work shows how neural architectures overcome these limitations through state maintenance mechanisms \cite{merrill2024}.
...
This directly supports our framework by demonstrating how memory mechanisms, not architectural complexity, determine computational power.
The key insight is that neural networks achieve complex computation not by circumventing their parallel nature, but by maintaining state across multiple processing steps.
...
Even direct protein-protein interactions and mechanical signal transduction operate through networks of molecules simultaneously changing conformational states \cite{alberts2022molecular}.
...
The CRISPR-Cas system exemplifies evolution's discovery of minimal yet effective memory architectures.
This remarkably efficient biological tape maintains a chronological record of viral infections, with new sequences inserted at one end \cite{sheth2017multiplex}.
The system achieves precise state maintenance through a two-protein mechanism that ensures exact repeat lengths and temporal ordering \cite{kim2019crispr}.
...
Even seemingly simple organisms like slime molds demonstrate sophisticated computation through physical memory structures in their extracellular matrix \cite{hoel2020emergence}, showing how memory-enabled computation can emerge from basic biological building blocks.
...
Our analysis reveals a fundamental pattern across scales: from molecular mechanisms to neural systems to artificial intelligence, advances in computational capability emerge from enhanced abilities to maintain and access state.
