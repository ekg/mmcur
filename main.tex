...
\begin{abstract}
Modern AI systems and biological cells share a fundamental computational constraint: their parallel architectures limit what computations they can perform in a single step. We show how both types of systems overcome this limitation through memory---the ability to maintain state and access computational history. First, we prove that any parallel system becomes capable of universal computation when equipped with these basic memory capabilities. We then demonstrate how this insight explains phenomena across artificial and biological systems, from the effectiveness of chain-of-thought prompting in language models to information processing in cells. Finally, we explore the practical implications, suggesting that the path to more capable AI systems may lie not in increasing model size, but in developing better mechanisms for memory and state maintenance.
\end{abstract}

\section{Introduction}
Adding memory to even simple computational systems can make them universal. This fundamental insight, dating back to Turing's original analysis of computation, shows that any system capable of basic state transitions becomes Turing-complete when equipped with memory. We demonstrate how this principle manifests in modern computational systems, from neural networks to biological cells.
