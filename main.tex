...
\begin{abstract}
Memory lies at the heart of computational universality.
The most basic universal computing system links a memory store to a state transition function.
Through rigorous mathematical analysis, we prove that any system with recursive state maintenance and reliable history access can simulate a Universal Turing Machine with logarithmic overhead.
This theoretical framework reveals fundamental constraints in parallel architectures, from neural networks to biological systems, where complex computation emerges through state maintenance rather than sophisticated processing units.
We demonstrate how modern language models achieve universal computation not through complex operations, but by learning to maintain and manipulate state through recursive processing.
This insight provides a unified perspective on computation across artificial and biological systems while suggesting new directions for AI architectures focused on robust state maintenance mechanisms.
\end{abstract}
