...
We began as collections of cells that evolved increasingly sophisticated ways to remember: from molecular markers that help cells maintain their identity, to immune systems that learn from encounters, to neural architectures that somehow give rise to consciousness.
...
A cell cannot afford to serialize its basic information processing---a typical eukaryotic cell contains gigabasepairs of DNA sequence, similar amounts of RNA, and billions of proteins \cite{milo2013protein} which are tumbling under Brownian motion in the gigahertz scale \cite{zhang2023molecular}, vibrating in the megahertz to terahertz range, and diffusing across the cell in seconds.
This is a space in which everything is literally interacting with everything else all the time, where structures appear ordered and planned only because their components have found each other through the rapid and continuous interaction of the entire cellular contents.
The physical nature of cellular systems makes sequential processing impractical---there's no efficient way to impose locks or ordering on billions of molecules in constant motion. This fundamental constraint forces parallel computation, just as training efficiency forces parallelism in artificial neural networks \cite{barrett2019analyzing}.
