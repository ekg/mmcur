...
\begin{abstract}
Memory is the secret sauce that makes parallel systems compute like universal Turing machines.
We prove that any system---from cells to neural networks---only needs two ingredients: the ability to remember its computational history and to use it recursively.
This explains why large language models can reason step-by-step despite being parallel processors, and how biological cells achieve complex computation through molecular memory rather than sophisticated processing.
The punchline? Don't build bigger computers---build better memory systems.
\end{abstract}
