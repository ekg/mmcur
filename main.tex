...
\begin{abstract}
We prove that any parallel computational system can achieve Turing-completeness through two minimal requirements: recursive state maintenance and reliable history access.
This theoretical result explains how seemingly limited systems---from cellular networks to neural architectures---achieve complex sequential computation despite being restricted to constant-depth parallel operations.
We demonstrate this principle in biological systems, where cells use molecular memory and developmental patterns to transcend the limitations of parallel chemical reactions, and in artificial intelligence, where language models leverage chain-of-thought reasoning to perform complex sequential computations through state maintenance.
Our framework unifies diverse phenomena across domains, from cellular information processing to recursive reasoning in AI, while providing concrete design principles for memory architectures in computational systems.
Through mathematical analysis and empirical evidence, we show that computational expressiveness emerges not from sophisticated processing units, but from simple mechanisms that maintain and access computational state across temporal boundaries.
These findings have immediate implications for AI system design, suggesting that advances in complex reasoning may depend more on improved state maintenance mechanisms than on larger or more sophisticated parallel processing units.
\end{abstract}
